...
AIThree::AIThree(char colour): AI(colour) {}

U8 AIThree::getType() { return 3; }

void AIThree::makeMove() {
  /*
  clearEP();
  ...
  while(true)
  {
    ...
    while (true && thePiece->getColour() == colour)
    {
      ...
      if (!(row1 == row2 && col1 == col2) && isValidMove(thePiece, row2, col2))
      {
        ...
        bool isUnderAttackBefore = gm->isCheck(row1, col1, colour);
        ...
        bool isCheck = gm->isCheck(c);
        bool isUnderAttackAfter = gm->isCheck(row2, col2, colour);
        ...
        if (!tmp && !isCheck && !(isUnderAttackBefore && !isUnderAttackAfter)
            || isUnderAttackAfter) // not a preferred move
        {
